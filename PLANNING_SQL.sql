-- ============================================
-- SCRIPT SQL POUR LE PLANNING INTELLIGENT
-- ============================================
-- Exécutez ce script APRÈS avoir créé les tables de base
-- ============================================

-- 1. Ajouter les nouvelles colonnes à la table appointments
ALTER TABLE appointments 
ADD COLUMN IF NOT EXISTS doctor_email TEXT,
ADD COLUMN IF NOT EXISTS appointment_type_id TEXT CHECK (appointment_type_id IN ('quick', 'normal', 'long')),
...
-- 2. Créer la table appointment_types (Types de rendez-vous)
CREATE TABLE IF NOT EXISTS appointment_types (
  id TEXT PRIMARY KEY CHECK (id IN ('quick', 'normal', 'long')),
  name TEXT NOT NULL,
...
INSERT INTO appointment_types (id, name, duration, description, doctor_email) VALUES
  ('quick', 'Consultation rapide', 10, 'Consultation courte de 10 minutes', NULL),
  ('normal', 'Consultation normale', 15, 'Consultation standard de 15 minutes', NULL),
  ('long', 'Consultation longue', 30, 'Consultation approfondie de 30 minutes', NULL)
ON CONFLICT (id) DO NOTHING;

-- 3. Créer la table doctor_planning_settings (Paramètres de planning)
CREATE TABLE IF NOT EXISTS doctor_planning_settings (
  ...
  base_slot_duration INTEGER NOT NULL DEFAULT 15,
  buffer_mode TEXT NOT NULL CHECK (buffer_mode IN ('per-consultations', 'per-hour')),
  buffer_frequency INTEGER NOT NULL DEFAULT 3,
  buffer_duration INTEGER NOT NULL DEFAULT 15,
  working_hours_start TEXT NOT NULL DEFAULT '09:00',
  working_hours_end TEXT NOT NULL DEFAULT '17:00',
  break_times JSONB, -- [{"start": "12:00", "end": "13:00"}]
  average_consultation_duration NUMERIC(5,2),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- 4. Créer la table appointment_sessions (Sessions de consultation)
CREATE TABLE IF NOT EXISTS appointment_sessions (
  ...
  duration INTEGER NOT NULL, -- durée prévue en minutes
...
-- 5. Créer la table appointment_statistics (Statistiques)
...
-- Paramètres de planning (médecin peut lire/modifier ses propres paramètres)
...
-- Sessions (tous peuvent lire/écrire pour le moment)
...
-- Statistiques (tous peuvent lire/écrire pour le moment)
